...
static int findLowestWeightCpu(void)
{
	int i = 0;
	int res = -1;
	unsigned int minWeight = 0xFFFFFFFF;
	struct wrrRq *rq;
...
void pullTaskWrr(struct rq *rq)
{
	...
	struct schedWrrEntity *se;
	struct wrrRq *tmp;

	if (!cpu_active(thisCpu))
		return;
	rcu_read_lock();
	for_each_online_cpu(i) {
		tmp = &cpu_rq(i)->wrr;
		if(i != thisCpu && tmp->nrRunning >=2 
		   && maxWeight < tmp->totalWeight) {
			maxWeight = tmp->totalWeight;
			targetCpu = i;		
		}
	}
	...
	if (maxWeight == -1)
		return;
	target = cpu_rq(targetCpu);
	double_rq_lock(thisRq,target);
	
	list_for_each_entry(se,&target->wrr.rq,runList) {
		tsk = list_entry(se,struct task_struct, wrr);
		if(task_running(target,tsk) &&
	 	   tsk->policy == SCHED_WRR &&
		   tsk->on_rq &&
		   cpumask_test_cpu(thisCpu, &tsk->cpus_allowed)) {
			deactivate_task(target,tsk,0);
			set_task_cpu(tsk,thisCpu);
			activate_task(thisRq,tsk,0);
			goto out;			
		}
	}	
out:
	double_rq_unlock(thisRq,target);
}
#endif
const struct sched_class wrrSchedClass = {
